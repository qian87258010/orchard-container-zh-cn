...
- 提供一键安装包：自动安装 Apple `container`，并安装星奕筑容器 UI。
...
推荐下载 GitHub Release 里的：

```text
Xingyizhu-Container-OneClick-Installer-1.12.1-zh.4-macos-arm64.zip
```
...
如果 macOS 阻止打开 `.command` 文件，请右键点击它，选择“打开”。

## 手动安装 Release 包

如果你只想使用图形界面，可以在 GitHub Release 下载 `Xingyizhu-Container-1.12.1-zh.4-macos-arm64.zip`。这个附件只包含星奕筑容器 UI，不包含 Apple `container`，所以仍需先安装 Apple `container`。
...
- Apple `container` 为 Apache-2.0 License，一键安装器通过 Homebrew 安装官方 `container` 包，本仓库不直接分发 Apple `container` 二进制文件。
